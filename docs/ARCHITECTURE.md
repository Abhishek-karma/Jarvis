    ...
    (Room Idempotency /       (Redacted Parameters /
     Durable State Store)        Persistent Ledger)
             ...
             └─────────────┬─────────────┘
                           ▼
                      Tool.execute()
   NeedleAction • MediaTools • System • Scoped Storage • Accessibility
                           │
                           ▼
                 Durable Observation
                           │
                           ▼
                      GoalEngine
           (Observation Verification / Next Turn)
```
...
### `GoalEngine` (LLM Cognitive Planner & Verifier)
- **Role**: High-level goal coordinator ensuring the **LLM is the central brain**.
- **Invariants**:
  - The LLM reasons over the intent, devises the multi-step plan, selects tools dynamically, and verifies the outcome before producing the final response.
  - Exposes `needle_action` as an on-device fast-path tool for instantaneous execution of device operations.
...
### `NeedleEngine` & `NeedleRouter` (On-Device Fast Action Engine)
- **Role**: Zero-overhead on-device tool matching for low-latency native actions (flashlight, volume, playback, settings, app launch).
- **Invariants**:
  - Exposed directly to the LLM via `needle_action` tool so the model decides when to delegate to on-device heuristics.
  - Returns verified observations back to the LLM.
...
- Implements Android system tools (Needle Action, Play Media, Media Control, Alarms, Calendar, Contacts, Device Controls, Files, System Info, Accessibility Automation, Communication) and Web search.
