...
### Fixed
- Local on-device agent no longer leaks raw `[[{"name":...,"args":{...}}]]` tool-call markup
  into the chat as the assistant's visible reply. New `ToolCallParser` (balanced JSON scan
  respecting string escapes, `name`/`args` validation, malformed-call rejection) backs a text
  fallback in `LocalLlmProvider`: text-embedded calls are converted into `ToolCallRequested`
  events so the tool actually executes, and the markup is stripped from the streamed prose.
  The local system prompt and text-prompt builder now also document the exact tool-call shape.
