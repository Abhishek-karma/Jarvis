...
class LocalLlmProviderTest {
    ...
    @Test
    fun `streams token deltas and a Done event from engine partials`() = runTest {
    ...
    @Test
    fun `maps engine failure to an Error event and never emits Done`() = runTest {
    ...
    @Test
    fun `capabilities advertise a tools-capable agent model`() = runTest {
        ...
        assertEquals(spec.id, provider.listModels().getOrNull()!!.single().id)
    }

    @Test
    fun `parses a structured tool call into a ToolCallRequested in agent mode`() = runTest {
        val engine = FakeEngine(
            partials = listOf(
        ...
        assertTrue(events.filterIsInstance<ChatStreamEvent.TokenDelta>().joinToString("") { it.text }
            .contains("I'll check"))
    }

    @Test
    fun `plain chat still streams a bare response without tool events`() = runTest {
        val provider = LocalLlmProvider(id = "local-gemma", spec = spec, engine = FakeEngine())
        val events = provider.streamChat(request()).toList()
