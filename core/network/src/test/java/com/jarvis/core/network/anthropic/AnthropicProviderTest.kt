...
class AnthropicProviderTest {
    ...
    @Test
    fun `streamChat emits Usage with Anthropic token counts`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
    ...
    @Test
    fun `streamChat sends Anthropic headers`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
    ...
    @Test
    fun `streamChat emits ToolCallRequested from streamed tool_use block`() =
        runTest {
    ...
    @Test
    fun `streamChat sends tools with input_schema objects in the body`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
    ...
    @Test
    fun `streamChat round-trips tool_use and tool_result turns in the body`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
    ...
    @Test
    fun `streamChat merges consecutive same-role messages into one wire message`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
            ...
            val history =
                listOf(
                    ...
                    Message(
                        ...
                        content = "Unknown tool \"nope\". Try again.",
                    ),
                )
    ...
    @Test
    fun `think mode enables extended thinking with raised max tokens`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))

            provider.streamChat(textRequest().copy(reasoningRequested = true)).toList()

            val body = recordedBody()
            val thinking = body["thinking"] as Map<*, *>
            assertEquals("enabled", thinking["type"])
            assertEquals(2048, (thinking["budget_tokens"] as Number).toInt())
            assertEquals(8192, (body["max_tokens"] as Number).toInt())
        }

    @Test
    fun `no think mode omits thinking config`() =
        runTest {
            val fixture = javaClass.classLoader!!.getResource("fixtures/anthropic_stream_text.txt")!!.readText()
            mockServer.enqueue(sseResponse(fixture))
            ...
            assertEquals(4096, (body["max_tokens"] as Number).toInt())
        }
