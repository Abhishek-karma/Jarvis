        ...
        callbackFlow {
            val dto =
                MessagesRequestDto(
                    ...
                    maxTokens = if (request.reasoningRequested) THINKING_MAX_TOKENS else 4096,
                    thinking =
                        if (request.reasoningRequested) {
                            ThinkingConfigDto(type = "enabled", budgetTokens = THINKING_BUDGET_TOKENS)
                        } else {
                            null
                        },
                            ...
                            "content_block_start" -> {
                                ...
                                if (block != null && block.type == "tool_use") {
                                    block.name?.let { toolName ->
                                        toolBlocks[event.index ?: toolBlocks.size] =
                                            ToolUseBuffer().apply {
                    ...
                    override fun onFailure(
                        ...
                        val errorMessage = when {
                            response != null -> {
                                ...
                                response.close()
                                when (response.code) {
                                    400 -> bodyText?.takeIf { it.isNotBlank() } ?: "Bad request — verify parameters"
                                    401 -> "Authentication failed — check your API key"
                                    403 -> "Access denied — your API key may lack necessary permissions"
                                    404 -> "Model not found — verify the selected model ID"
                                    429 -> "Rate limited / Quota exceeded — please try again in a few moments"
                                    500, 502, 503, 504 -> "Server error (${response.code}) — service temporarily unavailable"
                                    else -> bodyText?.takeIf { it.isNotBlank() } ?: (response.message.ifBlank { "HTTP ${response.code}" })
                                }
                            }
                            t is IOException -> "Network error — check your internet connection: ${t.message}"
...
data class ThinkingConfigDto(
    val type: String,
    @Json(name = "budget_tokens") val budgetTokens: Int? = null,
)


private const val THINKING_BUDGET_TOKENS = 2048
