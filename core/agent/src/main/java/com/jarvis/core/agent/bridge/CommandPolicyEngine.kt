...
class CommandPolicyEngine(
    ...
    /**
     * Evaluates the safety level of any typed operation.
     * Non-shell operations are vetted through their canonical shell form so that
     * typed privileges can never bypass the static denylist (e.g. a TypedOp that
     * would translate to `settings put global device_provisioned 0` is still blocked).
     */
    fun evaluateTyped(op: TypedOp): PolicyEvaluation = when (op) {
        is TypedOp.ShellCommand -> evaluate(op.command)
        is TypedOp.Screenshot, is TypedOp.InputGesture -> PolicyEvaluation(
            classification = PolicyClassification.ALLOWED,
            reason = "Read-only observation / UI interaction operation.",
        )
        else -> evaluate(op.toShellCommand())
    }
