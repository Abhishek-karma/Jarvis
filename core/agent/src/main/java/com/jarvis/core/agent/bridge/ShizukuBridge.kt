...
class ShizukuBridge(
    ...
    override suspend fun isAvailable(): BridgeStatus = withContext(Dispatchers.IO) {
        ...
        // 2. In standard runtime without binder connection, check running state
        // If a custom command runner is injected (e.g. in tests or active Shizuku binder session), test ping
        if (commandRunner != null) {
            try {
                ...
                return@withContext BridgeStatus.SERVICE_STOPPED
            }
        }

        // If package is installed but service binder is not yet initialized:
        BridgeStatus.SERVICE_STOPPED
    }
