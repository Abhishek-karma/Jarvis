...
/**
 * Canonical shell representation of a typed operation.
 * Single source of truth shared by the policy engine (pre-execution vetting)
 * and the Shizuku bridge (actual execution).
 */
fun TypedOp.toShellCommand(): String = when (this) {
    is TypedOp.GrantPermission -> "pm grant $packageName $permission"
    is TypedOp.RevokePermission -> "pm revoke $packageName $permission"
    is TypedOp.SetGlobalSetting -> "settings put global $key $value"
    is TypedOp.SetSecureSetting -> "settings put secure $key $value"
    is TypedOp.ForceStop -> "am force-stop $packageName"
    is TypedOp.SetAppEnabled -> "pm ${if (enabled) "enable" else "disable"} $packageName"
    is TypedOp.AppOp -> "cmd appops set $packageName $op $mode"
    ...
    is TypedOp.ShellCommand -> command
}
