...
class AgentEngine(
    ...
    fun run(request: AgentRunRequest): Flow<AgentEvent> = flow {
        ...
        while (steps < stepCap) {
            ...
            val streamEvents = request.provider.streamChat(
                ChatRequest(
            ...
            if (tool == null) {
                emit(AgentEvent.ToolRejected(call.name, "Unknown tool. Available tools: ${definitions.joinToString { it.name }}."))
                turnLog += assistantMessage(assistantText)
                turnLog += userMessage("Unknown tool \"${call.name}\". Available tools: ${definitions.joinToString { it.name }}.")
                continue
            }
            ...
            when (val validation = validator.validate(tool.parametersSchemaJson, call.argsJson)) {
                is ToolArgsValidator.Result.Rejected -> {
                    ...
                    turnLog += assistantMessage(assistantText)
                    turnLog += userMessage("Tool \"${tool.name}\" rejected its arguments: ${validation.reason} Fix the arguments and retry.")
                    continue
                }
    ...
    private fun toolMessage(observation: String, toolCallId: String, toolCallName: String) = Message(
        conversationId = "",
    ...
    private fun userMessage(note: String) = Message(
        conversationId = "",
...
sealed class AgentEvent {
    ...
    data class ToolExecuted(val name: String, val success: Boolean, val observationText: String) : AgentEvent()
    data class ToolRejected(val name: String, val reason: String) : AgentEvent()
