...
object CalendarTools {
    ...
    fun all(
        ...
        updateEvent: (suspend (eventId: Long, draft: CalendarEventDraft) -> Result<Unit>)? = null,
        deleteEvent: (suspend (eventId: Long) -> Result<Unit>)? = null,
    ): List<Tool> = buildList {
        add(createEvent(insertEvent))
        add(listEvents(queryEvents))
        add(setReminder(insertReminder))
        if (updateEvent != null) add(updateEvent(updateEvent))
        if (deleteEvent != null) add(deleteEvent(deleteEvent))
    }
        ...
        object : Tool {
            ...
            override suspend fun execute(argsJson: String): ToolResult {
                ...
                val start = args.long("start_utc_millis")
                    ?: args.long("startUtcMillis")
                    ?: args.long("start")
                val end = args.long("end_utc_millis")
                    ?: args.long("endUtcMillis")
                    ?: args.long("end")
        ...
        object : Tool {
            ...
            override suspend fun execute(argsJson: String): ToolResult {
                ...
                val from = args.long("from_utc_millis")
                    ?: args.long("fromUtcMillis")
                    ?: args.long("from")
                    ?: args.long("start")
                val to = args.long("to_utc_millis")
                    ?: args.long("toUtcMillis")
                    ?: args.long("to")
                    ?: args.long("end")
    ...
    fun updateEvent(update: suspend (Long, CalendarEventDraft) -> Result<Unit>): Tool =
        object : Tool {
            override val name = UPDATE_EVENT
            override val description =
                "Update an existing calendar event by ID with new title, times, description, or location. Sensitive: modifies user calendar."
            override val tier = PermissionTier.SENSITIVE
            override val parametersSchemaJson = UPDATE_EVENT_SCHEMA

            override suspend fun execute(argsJson: String): ToolResult {
                val args = Args.parse(argsJson) ?: return parseFailure()
                val eventId = args.long("event_id") ?: return missing("event_id is required")
                val title = args.string("title") ?: return missing("title is required")
                val start = args.long("start_utc_millis") ?: return missing("start_utc_millis is required")
                val end = args.long("end_utc_millis") ?: return missing("end_utc_millis is required")
                if (end < start) {
                    return ToolResult(
                        success = false,
                        observationText = "Event end time is before its start time.",
                ...
                return update(
                    ...
                    onSuccess = {
                        ToolResult(
                            success = true,
                            observationText = "Event $eventId updated.",
                            structuredData = mapOf("eventId" to eventId),
                        )
                    },
                    onFailure = { error ->
                        ToolResult(
                            success = false,
                            observationText = "Could not update event $eventId.",
    ...
    fun deleteEvent(delete: suspend (Long) -> Result<Unit>): Tool =
        object : Tool {
            override val name = DELETE_EVENT
            override val description =
                "Delete a calendar event by ID. Sensitive: deletes an item from the user's calendar."
            override val tier = PermissionTier.SENSITIVE
            override val parametersSchemaJson = DELETE_EVENT_SCHEMA

            override suspend fun execute(argsJson: String): ToolResult {
                val args = Args.parse(argsJson) ?: return parseFailure()
                val eventId = args.long("event_id") ?: return missing("event_id is required")
                return delete(eventId).fold(
                    onSuccess = {
                        ToolResult(
                            success = true,
                            observationText = "Event $eventId deleted.",
                            structuredData = mapOf("eventId" to eventId),
                        )
                    },
                    onFailure = { error ->
                        ToolResult(
                            success = false,
                            observationText = "Could not delete event $eventId.",
                            error = error.message ?: "Calendar delete failed",
                        )
                    },
                )
            }
    ...
    internal const val STALE_GRACE_MILLIS = 60_000L
    ...
    private const val UPDATE_EVENT_SCHEMA =
        """{"type":"object","properties":{"event_id":{"type":"integer"},"title":{"type":"string"},"description":{"type":"string"},"start_utc_millis":{"type":"integer"},"end_utc_millis":{"type":"integer"},"location":{"type":"string"}},"required":["event_id","title","start_utc_millis","end_utc_millis"]}"""
    private const val DELETE_EVENT_SCHEMA =
