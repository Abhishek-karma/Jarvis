...
class AlarmAndMediaToolsTest {
    ...
    fun `both tools are reversible-write tier`() {
        ...
        assertEquals(PermissionTier.REVERSIBLE_WRITE, AlarmTools.setTimer { _, _ -> Result.success(Unit) }.tier)
    ...
    @Test
    fun `set_timer starts countdown and validates positive duration`() =
        runBlocking {
            var capturedSeconds: Int? = null
            var capturedLabel: String? = null
            val tool =
                AlarmTools.setTimer { seconds, label ->
                    capturedSeconds = seconds
                    capturedLabel = label
                    Result.success(Unit)
                }

            val bad = tool.execute("""{"duration_seconds": -5}""")
            assertFalse(bad.success)
            assertTrue(bad.error!!.contains("duration_seconds"))

            val good = tool.execute("""{"duration_seconds": 300, "label": "Boil eggs"}""")
            assertTrue(good.success)
            assertEquals(300, capturedSeconds)
            assertEquals("Boil eggs", capturedLabel)
        }

    @Test
    fun `play_media executes with search query and app name`() =
        runBlocking {
            var capturedQuery: String? = null
            var capturedApp: String? = null
            val tool = MediaTools.playMedia { q, app ->
                capturedQuery = q
                capturedApp = app
                Result.success("Playing \"$q\" on $app.")
            }

            val result = tool.execute("""{"query":"jazz","app_name":"spotify"}""")
            assertTrue(result.success)
            assertEquals("jazz", capturedQuery)
            assertEquals("spotify", capturedApp)
            assertTrue(result.observationText.contains("Playing \"jazz\" on spotify."))
        }

    @Test
    fun `media_control validates command action`() =
        runBlocking {
            var capturedAction: String? = null
            val tool = MediaTools.mediaControl { act ->
                capturedAction = act
                Result.success("Media command '$act' dispatched.")
            }

            val bad = tool.execute("""{"action":"fastforward"}""")
            assertFalse(bad.success)

            val good = tool.execute("""{"action":"pause"}""")
            assertTrue(good.success)
            assertEquals("pause", capturedAction)
        }
}
