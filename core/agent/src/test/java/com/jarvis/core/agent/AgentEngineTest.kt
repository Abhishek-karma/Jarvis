...
class AgentEngineTest {
    ...
    @Test
    fun `direct answer without tool calls ends the run`() = runTest {
        val audit = RecordingAudit()
        val provider = FakeLlmProvider(script = { listOf(ChatStreamEvent.TokenDelta("Sure."), ChatStreamEvent.Done) })
    ...
    @Test
    fun `read-only tool executes without confirmation then the model answers`() = runTest {
        ...
        assertTrue(provider.requests[1].conversationHistory.any { it.role == MessageRole.TOOL && it.content == "ok" })
    }

    @Test
    fun `sensitive tool waits for confirmation and records userConfirmed when granted`() = runTest {
        ...
        assertTrue(events.contains(AgentEvent.ConfirmationRequired("send_email", """{"to": "a@b.c", "body": "hello"}""")))
        assertEquals(1, tool.executions)
    ...
    @Test
    fun `denied sensitive call is cancelled and never executed`() = runTest {
        val tool = FakeTool("send_email", PermissionTier.SENSITIVE)
        val registry = ToolRegistry().apply { register(tool) }
        val audit = RecordingAudit()
        ...
        assertEquals(listOf(AgentEvent.ToolCancelled("send_email")), events.filterIsInstance<AgentEvent.ToolCancelled>())
    ...
    @Test
    fun `unknown tool is surfaced and the loop continues`() = runTest {
    ...
    @Test
    fun `invalid args are rejected so the model can retry, then execute succeeds`() = runTest {
        val tool = FakeTool(
            "battery",
        ...
        val provider = FakeLlmProvider().apply {
            script = {
                when (requests.size) {
                    ...
                    2 -> listOf(ChatStreamEvent.ToolCallRequested("battery", """{"level": 1}"""), ChatStreamEvent.Done)
    ...
    @Test
    fun `step cap reached ends the run with partial progress`() = runTest {
        val tool = FakeTool("battery", PermissionTier.READ_ONLY)
        val registry = ToolRegistry().apply { register(tool) }
        val audit = RecordingAudit()
        val provider = FakeLlmProvider(script = {
    ...
    @Test
    fun `provider stream error fails the run`() = runTest {
        val provider = FakeLlmProvider(script = {
            listOf(ChatStreamEvent.Error(code = "429", message = "rate limited", retryable = true), ChatStreamEvent.Done)
        })
        val audit = RecordingAudit()
        val engine = engine(ToolRegistry(), audit)
    ...
    @Test
    fun `cancelling a run lets an in-flight tool call finish and still audits it`() = runTest(UnconfinedTestDispatcher()) {
        ...
        val tool = object : Tool {
            ...
            override val parametersSchemaJson = """{"type":"object","properties":{},"required":[]}"""
    ...
    private fun request(provider: FakeLlmProvider) = AgentRunRequest(
        provider = provider,
        modelId = "test-model",
