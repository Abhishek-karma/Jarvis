...
class AgentEngineTest {
    ...
    fun `direct answer without tool calls ends the run`() =
        runTest {
            ...
            assertEquals(null, provider.requests.single().toolsAvailable)
            assertTrue(audit.records.isEmpty())
        }

    @Test
    fun `disabled tools are never advertised to the model`() =
        runTest {
            val battery = FakeTool("battery", PermissionTier.READ_ONLY)
            val fetch = FakeTool("fetch_url", PermissionTier.READ_ONLY)
            val registry =
                ToolRegistry().apply {
                    register(battery)
                    register(fetch)
            ...
            val engine =
                AgentEngine(registry, RecordingAudit(), RecordingGate(), disabledTools = setOf("fetch_url"))

            engine.run(request(provider)).toList()

            val advertised = provider.requests.single().toolsAvailable.orEmpty()
            assertTrue(advertised.any { it.name == "battery" })
            assertFalse(advertised.any { it.name == "fetch_url" })
            assertEquals(0, fetch.executions)
            assertEquals(0, battery.executions)
        }
    ...
    fun `sensitive tool waits for confirmation and records userConfirmed when granted`() =
        runTest {
            ...
            audit.records.single().let {
                ...
                assertFalse(it.paramsRedactedJson.contains("hello"))
    ...
    @Test
    fun `cancelling a run cancels in-flight tool call and still audits it as cancelled`() =
        runTest(UnconfinedTestDispatcher()) {
            ...
            assertEquals(1, audit.records.size)
            assertEquals("cancelled", audit.records.single().resultStatus)
        }
    ...
    @Test
    fun `multi-tool execution runs read-only tools concurrently and collects all results`() =
        runTest {
            val toolA = FakeTool("get_time", PermissionTier.READ_ONLY)
            val toolB = FakeTool("get_battery", PermissionTier.READ_ONLY)
            ...
            val provider =
                FakeLlmProvider().apply {
                    script = { request ->
                        if (requests.size == 1) {
                            listOf(
                                ChatStreamEvent.ToolCallRequested("get_time", """{}"""),
                                ChatStreamEvent.ToolCallRequested("get_battery", """{}"""),
                                ChatStreamEvent.Done,
                            )
                        } else {
                            listOf(ChatStreamEvent.TokenDelta("Time is 10:00 and battery is 90%."), ChatStreamEvent.Done)
                        }
                    }
                }
            val engine = engine(registry, audit)
            ...
            assertEquals(2, audit.records.size)
            val finalAnswer = events.filterIsInstance<AgentEvent.FinalAnswer>().single()
            assertEquals("Time is 10:00 and battery is 90%.", finalAnswer.text)
        }
