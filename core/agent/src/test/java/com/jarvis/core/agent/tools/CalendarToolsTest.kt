...
class CalendarToolsTest {
    ...
    fun `tiers are declared per spec`() {
        ...
        assertEquals(PermissionTier.SENSITIVE, CalendarTools.updateEvent { _, _ -> Result.success(Unit) }.tier)
        assertEquals(PermissionTier.SENSITIVE, CalendarTools.deleteEvent { Result.success(Unit) }.tier)
    }
    ...
    @Test
    fun `update_event updates and reports success`() =
        runBlocking {
            var updatedId: Long? = null
            var updatedDraft: CalendarTools.CalendarEventDraft? = null
            val tool =
                CalendarTools.updateEvent { id, draft ->
                    updatedId = id
                    updatedDraft = draft
                    Result.success(Unit)
                }
            ...
            val result =
                tool.execute(
                    """{"event_id":42,"title":"New Title","start_utc_millis":$dayStart,"end_utc_millis":${dayStart + 1800000},"location":"Room 2"}""",
                )

            assertTrue(result.success)
            assertEquals(42L, updatedId)
            assertEquals("New Title", updatedDraft?.title)
            assertEquals("Room 2", updatedDraft?.location)
        }

    @Test
    fun `delete_event deletes by id`() =
        runBlocking {
            var deletedId: Long? = null
            val tool =
                CalendarTools.deleteEvent { id ->
                    deletedId = id
                    Result.success(Unit)
                }

            val result = tool.execute("""{"event_id":99}""")

            assertTrue(result.success)
            assertEquals(99L, deletedId)
        }
}
