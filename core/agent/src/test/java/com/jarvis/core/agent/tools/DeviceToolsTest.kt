...
class DeviceToolsTest {
    ...
    @Test
    fun `list_installed_apps executes and filters query`() = runBlocking {
        var queryReceived: String? = null
        val tool = DeviceTools.listInstalledApps { q ->
            queryReceived = q
            Result.success(listOf("WhatsApp (com.whatsapp)", "YouTube (com.google.android.youtube)"))
        }

        val allRes = tool.execute("""{}""")
        assertTrue(allRes.success)
        assertTrue(allRes.observationText.contains("WhatsApp"))
        assertTrue(allRes.observationText.contains("YouTube"))

        val filteredRes = tool.execute("""{"query":"YouTube"}""")
        assertTrue(filteredRes.success)
        assertEquals("YouTube", queryReceived)
    }

    @Test
    fun `install_app validates argument and opens play store`() = runBlocking {
        var installTarget = ""
        val tool = DeviceTools.installApp { app ->
            installTarget = app
            Result.success(Unit)
        }

        val empty = tool.execute("""{}""")
        assertFalse(empty.success)

        val valid = tool.execute("""{"app_name":"YouTube"}""")
        assertTrue(valid.success)
        assertEquals("YouTube", installTarget)
        assertTrue(valid.observationText.contains("Google Play Store"))
    }
}
