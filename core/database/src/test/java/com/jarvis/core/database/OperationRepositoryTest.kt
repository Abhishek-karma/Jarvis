...
class OperationRepositoryTest {
    ...
    @Test
    fun `updateStatus transitions EXECUTING to UNKNOWN with uncertain reason`() = runTest {
        val original = op()
        repo.insert(original)
        repo.updateStatus(original.copy(
        ...
        val updated = repo.getByKey("idem-key-1")!!
        assertEquals(OperationStatus.UNKNOWN, updated.status)
        assertEquals("Interrupted by app shutdown; outcome is uncertain.", updated.errorMessage)
    }
