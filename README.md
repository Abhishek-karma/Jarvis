...
**Jarvis** is an open-source, privacy-first mobile AI agent platform engineered for Android. It bridges frontier cloud models (Gemini, Claude, OpenAI, Ollama, OpenRouter, Groq) and on-device neural reasoning with native Android capabilities, instantaneous on-device action delegation via Needle, elevated device control via Shizuku, resilient background automations via WorkManager, UI accessibility automation, and a zero-trust policy gating engine.

> **Guiding Principle**: *The LLM is the central brain and cognitive planner. The tools are powerful, sandboxed, and audited.*

---

## Key Features

- 🧠 **LLM-Driven Cognitive Planning (`GoalEngine`)**: The LLM functions as the central brain—formulating multi-step plans, deciding tool invocations dynamically, evaluating execution observations, and verifying task completion before concluding.
- ⚡ **Dynamic On-Device Delegation (`needle_action`)**: Low-latency, rule-based device intent execution embedded as a first-class tool (`needle_action`), empowering the LLM to delegate instant on-device actions (system settings, volume, flashlight, alarms, media playback) without round-trip latency.
- 🎵 **Comprehensive Media & Music Control**: Native `play_media` (supporting queries and app targeting such as Spotify, YouTube Music, Apple Music) and `media_control` (play, pause, stop, next, previous).
- 📱 **UI Automation & Accessibility Service (`JarvisAccessibilityService`)**: Structured view hierarchy inspection, accessibility node analysis, automated clicking, scrolling, text entry, and UI navigation.
- ⚙️ **Canonical Execution Loop (`AgentRunner`)**: Deterministic multi-turn tool-calling engine featuring strict JSON schema validation, sequential mutating actions, bounded step limits, token budget management, and cooperative coroutine cancellation.
...
- 💾 **Durable Idempotency Ledger**: SQLite/Room-backed state machine tracking every operation from initiation to completion. Guarantees zero duplicate side-effects across app recreation, background retries, and process death with crash recovery (`EXECUTING` → `UNKNOWN`).
...
- 🔄 **Background Automations**: Asynchronous research and scheduled background routines managed natively via AndroidX WorkManager.
...
```text
 ...
 └────────────────────────────────┬────────────────────────────────┘
                                  │ User Input / Assistant Goal
                                  ▼
 ┌─────────────────────────────────────────────────────────────────┐
 │                           GoalEngine                            │
 │            (LLM Cognitive Planning & Verification Loop)         │
 └────────────────────────────────┬────────────────────────────────┘
                                  │
                                  ▼
 ┌─────────────────────────────────────────────────────────────────┐
 ...
 │                                                                 │
 │   1. Model Inference (Remote SSE Streaming / Local Providers)   │
 ...
 ┌─────────────────────────────────────────────────────────────────┐
 │                         ToolRegistry                            │
 │  • needle_action (Fast On-Device) • play_media / media_control  │
 │  • search_web / fetch_url         • create_file / read_file     │
 │  • launch_app / system_settings   • shizuku_bridge / ui_action  │
 └──────────────┬───────────────────────────────────┬──────────────┘
                │                                   │
                ▼                                   ▼
 ┌──────────────────────────────┐   ┌──────────────────────────────┐
 ...
 │   Needle Engine • Hardware • Scoped Storage • Accessibility     │
 └────────────────────────────────┬────────────────────────────────┘
                                  │ Safe Observation Envelope
                                  ▼
 ┌─────────────────────────────────────────────────────────────────┐
 │        Prompt Continuation / Observation Verification Turn      │
 └─────────────────────────────────────────────────────────────────┘
```
...
├── core/
│   ├── agent/            # GoalEngine, AgentRunner, NeedleEngine, ToolPolicy, ToolRegistry
...
|---|---|---|---|
| **Google Gemini** | Cloud API | `gemini-2.5-flash`, `gemini-1.5-pro`, `gemini-1.5-flash` | Multimodal, Native Function Calling, Fast SSE Streaming |
| **Anthropic Claude**| Cloud API | `claude-3-5-sonnet`, `claude-3-haiku`, `claude-3-opus` | Multi-Step Reasoning, Native Tool Calling |
...
|---|---|---|---|
| `needle_action` | Fast On-Device | `READ_ONLY` / `ACTION` | Delegates instant on-device actions (settings, volume, alarms, media) to the Needle engine. |
| `play_media` | Media & Audio | `ACTION` | Plays songs, artists, playlists, or albums with query parsing and app targeting (Spotify, YouTube Music, Apple Music). |
| `media_control` | Media & Audio | `ACTION` | Controls ongoing media playback (play, pause, stop, next, previous). |
...
| `launch_app` | System | `ACTION` | Launches specified installed applications or activities. |
...
| `shizuku_bridge` | System / ADB | `SENSITIVE` | Executes elevated system tasks via Shizuku v13 IPC (requires explicit user confirmation). |
...
- 🧪 [LOCAL_AGENT_TESTING.md](docs/LOCAL_AGENT_TESTING.md) — Agent tool-calling verification guide.
