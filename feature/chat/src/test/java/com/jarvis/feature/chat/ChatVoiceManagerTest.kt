...
import com.jarvis.core.voice.SttErrorCategory
...
class ChatVoiceManagerTest {
    ...
    fun `speaking assistant response synthesizes tts and automatically loops back to listening`() = testScope.runTest {
        coEvery { ttsProvider.synthesize("Done!") } returns Result.success(
            TtsResult(
        ...
        coVerify { audioPlayer.play(byteArrayOf(1, 2, 3), "wav") }
    ...
    fun `interruptSpeaking stops playback and immediately resumes listening`() = testScope.runTest {
        coEvery { ttsProvider.synthesize(any()) } returns Result.success(
            TtsResult(
                audioData = byteArrayOf(1, 2, 3),
                format = "wav",
            )
    ...
    fun `prepareStreamingResponse and onStreamingToken speaks sentence chunks as they stream`() = testScope.runTest {
        coEvery { ttsProvider.synthesize("Hello world.") } returns Result.success(
            TtsResult(audioData = byteArrayOf(1, 2, 3), format = "wav")
        )
        ...
        coVerify { ttsProvider.synthesize("Hello world.") }
        coVerify { audioPlayer.play(byteArrayOf(1, 2, 3), "wav") }
        assertTrue(voiceStateMachine.state.value is VoiceSessionState.Listening)
    }

    @Test
    fun `barge-in interruptSpeaking invokes tts stop and restarts listening`() = testScope.runTest {
        voiceManager.startVoiceMode(
            scope = this,
            mainDispatcher = testDispatcher,
            onUserSpeechFinal = {},
        ...
        io.mockk.verify { ttsProvider.stop() }
        coVerify { audioPlayer.stop() }
        io.mockk.verify { liveSttSession.cancelListening() }
        assertTrue(voiceStateMachine.state.value is VoiceSessionState.Listening)
    }

    @Test
    fun `progressive streaming synthesizes multiple sentences and transitions to listening`() = testScope.runTest {
        coEvery { ttsProvider.synthesize("First sentence.") } returns Result.success(
            TtsResult(audioData = byteArrayOf(1, 2), format = "wav")
        )
        coEvery { ttsProvider.synthesize("Second sentence.") } returns Result.success(
            TtsResult(audioData = byteArrayOf(3, 4), format = "wav")
        )
        ...
        voiceManager.prepareStreamingResponse(scope = this, mainDispatcher = testDispatcher)
        voiceManager.onStreamingToken("First sentence. ")
        voiceManager.onStreamingToken("Second sentence. ")
        voiceManager.onStreamingComplete()
        testScheduler.advanceUntilIdle()

        coVerify { ttsProvider.synthesize("First sentence.") }
        coVerify { ttsProvider.synthesize("Second sentence.") }
        coVerify { audioPlayer.play(byteArrayOf(1, 2), "wav") }
        coVerify { audioPlayer.play(byteArrayOf(3, 4), "wav") }
        assertTrue(voiceStateMachine.state.value is VoiceSessionState.Listening)
    }

    @Test
    fun `transient STT error automatically retries listening without exiting voice mode`() = testScope.runTest {
        val errorSlot = slot<(SttRecognitionError) -> Unit>()
        every {
            liveSttSession.startListening(
                onPartial = any(),
                onResult = any(),
        ...
        // Simulate transient timeout / no-speech error
        errorSlot.captured.invoke(
            SttRecognitionError(
                category = SttErrorCategory.TRANSIENT,
                message = "No speech detected (code 7)",
                code = 7,
            )
        )
