...
class ChatViewModelTest {
    ...
    @Test
    fun `sendMessage does nothing when composer is empty`() = runTest {
    ...
    @Test
    fun `isSendingEnabled becomes false when no providers configured`() = runTest {
    ...
    @Test
    fun `isSendingEnabled becomes true when providers are configured`() = runTest {
    ...
    @Test
    fun `openConversationById loads the specified conversation`() = runTest {
    ...
    @Test
    fun `createNewConversation creates a new conversation`() = runTest {
    ...
    fun `setRoutingOverride persists override to conversation`() = runTest {
        ...
        val conversation = Conversation(id = "conv-r", title = "Routing Chat", routingOverride = RoutingOverride.AUTO)
    ...
    @Test
    fun `setRoutingOverride updates activeRoute via resolveRoute`() = runTest {
    ...
    @Test
    fun `setRoutingOverride to LOCAL emits ShowNotice`() = runTest {
    ...
    @Test
    fun `setRoutingOverride to CLOUD does not emit notice`() = runTest {
    ...
    @Test
    fun `openConversation restores routing override from conversation`() = runTest {
    ...
    fun `cancelStreaming marks STREAMING assistant message as STOPPED`() = runTest {
        ...
        assertEquals(MessageStatus.STREAMING, viewModel.uiState.value.messages[0].status)
    ...
    @Test
    fun `cancelStreaming with no active stream is a no-op`() = runTest {
        viewModel.cancelStreaming()
        advanceUntilIdle()
        assertFalse(viewModel.uiState.value.isStreaming)
    }
    ...
    fun `Jarvis prefix routes to agent mode and persists the final answer`() = runTest {
        ...
        coEvery { provider.streamChat(any()) } returns flowOf(
    ...
    @Test
    fun `agent run renders each canvas milestone with completed checkmarks`() = runTest {
        val provider = agentProvider(
            listOf(
                flowOf(ChatStreamEvent.ToolCallRequested(name = "current_time", argsJson = "{}"), ChatStreamEvent.Done),
    ...
    @Test
    fun `sensitive tool parks the run at the confirmation until allowed`() = runTest {
        val provider = agentProvider(
            listOf(
                flowOf(ChatStreamEvent.ToolCallRequested(name = "send_it", argsJson = "{\"text\":\"hi\"}"), ChatStreamEvent.Done),
    ...
    @Test
    fun `denying a sensitive tool halts the run and persists no assistant answer`() = runTest {
        val provider = agentProvider(
            listOf(
                flowOf(ChatStreamEvent.ToolCallRequested(name = "send_it", argsJson = "{\"text\":\"hi\"}"), ChatStreamEvent.Done),
            ),
